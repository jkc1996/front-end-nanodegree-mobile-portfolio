...
function changePizzaSizes(size){var randomPizzas=document.querySelectorAll(".randomPizzaContainer");for(var i=0;i<randomPizzas.length;i++){randomPizzas[i].style.width=newwidth;}}
...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");var items=document.getElementsByClassName('mover');var scrollTop=document.documentElement.scrollTop||document.body.scrollTop;var scrollVar=scrollTop/1250;var phase=[];for(var j=0;j<5;j++){phase.push(Math.sin(scrollVar+j));}
for(var i=0;i<items.length;i++){items[i].style.left=items[i].basicLeft+100*(phase[i%5])+'px';}
...
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=8;var s=256;var factor=s / cols;var movingPizza=document.getElementById("movingPizzas1");for(var i=0;i<100;i++){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=Math.floor(i*factor)+'px';movingPizza.appendChild(elem);}
updatePositions();});