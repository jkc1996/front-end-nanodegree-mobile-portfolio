...
var resizePizzas = function (size) {
  ...
  //Replaced the queryselector with getElementsById.because this api calls faster.
  ...
  /* First document.querySelectorAll(".randomPizzaContainer") causes the violation of DRY(don't repeat yourself).for that i created the new var called rendomPizzas.
   * Chrome devtools time line shows that the changePizzaSizes causes the Forced Sync Layout.
   * no need of dx inside this function because it is based on the size since there is only value of the size passed.
   * also only addidng 8% additional width newWidth is doing nothing.So i removed it.
   */
  function changePizzaSizes(size) {
    ...
    //Replace the queryselector with getElementsByClassName.because this api calls faster.
    //It is better to save the array length in a local variable, so the array's
    //length property is not accessed to check its value at each iteration. (i.e. more efficiency)
...
/*
 * I found that background pizza causing lots of problem for our 60 fps.
 * So I Defined a new variable called scrollTop to store the no. of pixels bar was scrolled to the top
 * Too much phases of pizzas also creating the our frame rate.for that i defined phase as an
 * array variable to store only 5 values and moved it outside of the loop.
...
function updatePositions() {
  ...
  var phase = [];
  // Take the phase out of the main for loop and pushing 5 values.
  for (var j = 0; j < 5; j++) {
    phase.push(Math.sin(scrollVar + j));
  }
...
/*Replace querySelector with getElementById and moved it outside the loop.
 * Also reduced the no pizzas to 24 (i.e 3 full lines of 8 background pizza).
 */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  //Declaring the elem variable in the initialisation of the for-loop will
